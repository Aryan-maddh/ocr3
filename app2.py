    ...
    for model, keywords in MODEL_MAPPINGS.items():
        ...
        for keyword in keywords:
            if keyword in message:
                if f" {keyword} " in f" {message} " or message.startswith(keyword) or message.endswith(keyword):
...
def translate_to_english(text):
    try:
        result = subprocess.run(
            [
                "../venv_googletrans/bin/python",  # Adjust path if needed
        ...
        if output.get("success"):
            ...
            print("Translation error:", output.get("error"))
            return text
    except Exception as e:
